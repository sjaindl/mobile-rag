...
class GetAssistantCompletionUseCase(private val repository: AssistantRepository) {
    operator fun invoke(prompt: String, chatId: String?, streaming: Boolean): Flow<FlowiseResponse> {
        return repository.getCompletion(prompt = prompt, chatId = chatId, streaming = streaming)
    }
}
